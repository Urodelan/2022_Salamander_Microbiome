...
densities<-read.csv(file="../FromCluster/RegionITS_NB_Reg_Density_Predictions.csv",stringsAsFactors=F)
...
for(i in 1:max(unique_taxa$Subset)){
  ...
  DensitiesPlot<-ggplot(densities_taxa_to_plot,aes(x=Date_numeric,y=Quantile_0.5,color=Life_Stage))+
    ...
    ggtitle(paste0("Fungal Densities on Salamander Skin (Plot ",i,
  ...
  # Save densities plot.
  ggsave(filename=paste0("DensitiesPlot_ITS_",i,".pdf"),device=cairo_pdf,plot=DensitiesPlot,width=21,height=21)
