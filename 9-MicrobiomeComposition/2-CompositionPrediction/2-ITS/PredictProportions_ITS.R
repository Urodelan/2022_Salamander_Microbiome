##################################################################################
### Predict Proportions From ITS Dirichlet-Multinomial Regression Model Output ###
##################################################################################
...
working_directory<-"/uufs/chpc.utah.edu/common/home/gompert-group2/data/kg_microbes/Scripts/Models/Output/RegionITS"
...
for(i in c("Salamander","Water","Substrate")){
  ...
  reads<-read.csv(
    paste0(composition_model_directory,"/",i,"/Region",barcode_region,"/Take3/",i,"_ASVTable_ITS_Comp_2.csv"),
    row.names=1,check.names=F)
  
  # Load scaling data.
  print(paste("-- Loading",tolower(i),"scaling data."))
  stratum_scaling<-read.csv(
    paste0(composition_model_directory,"/",i,"/Region",barcode_region,"/Take3/stratum_scaling.csv"),
    row.names=1)
  pred_scaling<-read.csv(
    paste0(composition_model_directory,"/",i,"/Region",barcode_region,"/Take3/pred_scaling.csv"))
  ...
  if(i=="Salamander"){
    # Load the model selection 2 data for salamanders, since variable selection required a single
    # continuation to complete.
    model_selection<-read.csv(
      paste0(composition_model_directory,"/",i,"/Region",barcode_region,"/Take3/model_selection2.csv"),
      check.names=F)
  } else {
    model_selection<-read.csv(
      paste0(composition_model_directory,"/",i,"/Region",barcode_region,"/Take3/model_selection.csv"),
      check.names=F)
  }
  ...
  load(paste0(composition_model_directory,"/",i,"/Region",barcode_region,"/Take3/mod_fit.RData"))
