########################################
### Plot ITS Proportional Abundances ###
########################################
...
props<-read.csv(file="../FromCluster/Predictions/props_summary_ITS.csv",stringsAsFactors=F)
...
for(i in 1:max(unique_taxa$Subset)){
  ...
  PropsPlot<-ggplot(props_sal_taxa_to_plot,aes(x=Date_numeric,y=Quantile_0.5,color=Life_Stage))+
    ...
    ggtitle(paste0("Fungal Proportional Abundances on Salamander Skin (Plot ",i,
  ...
  ggsave(filename=paste0("PropsPlot_ITS_",i,".pdf"),device=cairo_pdf,plot=PropsPlot,width=21,height=21)
