...
hills<-read.csv(file="../FromCluster/Predictions/hills2_summary_16S.csv")
...
hillsPlot<-ggplot(hills,aes(x=Date_numeric,y=Quantile_0.5,color=Life_Stage))+
  ...
  ggtitle("Bacterial Hill's Diversity Index (\u03B1 = 2) on Salamander Skin",
...
ggsave(filename="HillsPlot_16S.pdf",device=cairo_pdf,plot=hillsPlot,width=7.5,height=5)

# Load Bray-Curtis dissimilarity.
load("../FromCluster/Predictions/bc_summary_16S.RData")
...
BC_title<-ggdraw()+
  draw_label("Bacterial Bray-Curtis Dissimilarity Through Time on Salamander Skin",fontface="bold",
...
ggsave("BrayCurtisPlot_16S.pdf",device=cairo_pdf,plot_BC,width=14,height=15)
