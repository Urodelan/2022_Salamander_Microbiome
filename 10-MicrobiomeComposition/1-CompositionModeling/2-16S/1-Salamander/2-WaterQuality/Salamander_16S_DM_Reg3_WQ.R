##########################################################################################
### Fit Water Quality Dirichlet-Multinomial Regression Model with Salamander 16S Reads ###
##########################################################################################
...
working_directory<-"/uufs/chpc.utah.edu/common/home/gompert-group2/data/kg_microbes/Scripts/Models/Composition/Salamander/Region16S/WQ"

# Declare path to reads in-file (with .csv extension).
reads_in_file<-"../Take2/Salamander_ASVTable_16S_Comp_2.csv"
...
# Get data for filling in DO (ppm) NAs from the faulty DO meter on Aug. 11th at Ponds Lake by
# taking the average of the DO (ppm) values at each stratum for the weeks sampled immediately
# before and after at Ponds Lake.
address_DO_NAs<-data.frame("Stratum"=4+c(1,2,3,4,5),
                           "PPM"=c(5.5,4.195,5.51,5.97,4.765))

# Loop through each record in the metadata.
for(i in 1:nrow(metadata)){
  # Check to see if DO (ppm) is NA.
  if(is.na(metadata[i,"Dissolved_oxygen_ppm"])){
    # If so, add in our best estimate of DO (ppm) for the missing observations.
    metadata[i,"Dissolved_oxygen_ppm"]<-
      address_DO_NAs$PPM[address_DO_NAs$Stratum==metadata[i,"Stratum"]]
  }
}

# Check that no NAs remain in the metadata for DO (ppm).
if(any(is.na(metadata$Dissolved_oxygen_ppm))) stop("NAs remain in DO (ppm).")

# Using all four water quality variables as predictors with interactions produces
# more predictors than reasonable for backwards variable selection.
# Pearson correlation coefficient is highest between temperature and conductivity (0.8166319).
# We will exclude conductivity as a predictor.
#cor(metadata[,c("Temperature_C","pH","Conductivity_uS","Dissolved_oxygen_ppm")])

# Get just LM and water quality variables.
pred_matrix<-metadata[,c("Age_numeric",
                         "LM_binary",
                         "Temperature_C","pH",
                         # "Conductivity_uS",
...
# Get mean and SD of temperature.
temp_mean<-mean(pred_matrix$Temperature_C)
temp_sd<-sd(pred_matrix$Temperature_C)

# Get mean and SD of pH.
pH_mean<-mean(pred_matrix$pH)
pH_sd<-sd(pred_matrix$pH)

# # Get mean and SD of conductivity.
# cond_mean<-mean(pred_matrix$Conductivity_uS)
# cond_sd<-sd(pred_matrix$Conductivity_uS)

# Get mean and SD of DO.
DO_mean<-mean(pred_matrix$Dissolved_oxygen_ppm)
DO_sd<-sd(pred_matrix$Dissolved_oxygen_ppm)
...
pred_matrix_scaled<-data.frame(Age=(pred_matrix$Age_numeric-age_mean)/age_sd,
                               ...
                               Temp=(pred_matrix$Temperature_C-temp_mean)/temp_sd,
                               pH=(pred_matrix$pH-pH_mean)/pH_sd,
                               # Cond=(pred_matrix$Conductivity_uS-cond_mean)/cond_sd,
                               DO_ppm=(pred_matrix$Dissolved_oxygen_ppm-DO_mean)/DO_sd)

# Add interaction terms to the scaled predictor matrix.
pred_matrix_scaled<-as.data.frame(model.matrix(~Age*LM*Temp*pH*DO_ppm-1,data=pred_matrix_scaled))

# Collect non-stratum predictor scaling parameters into a data frame.
pred_scaling<-data.frame(Predictor=c("Age","Age","LM",
                                     "Temp","Temp",
                                     "pH","pH",
                                     "DO_ppm","DO_ppm"),
                         Parameter=c("Mean","SD","Mean",
                                     "Mean","SD",
                                     "Mean","SD",
                                     "Mean","SD"),
                         Value=c(age_mean,age_sd,LM_mean,
                                 temp_mean,temp_sd,
                                 pH_mean,pH_sd,
                                 DO_mean,DO_sd))
...
for(i in 1:nrow(reads)){
  ...
  for(k in 1:ncol(reads)){
    ...
    eta[,k]<-intercept_betas[,k]+as.matrix(pred_taxon_betas) %*% t(pred_values)
...
## Create data frame.
df<-as.data.frame(matrix(ncol=2+ncol(pred_matrix_scaled)+5))
colnames(df)<-c("Set","Model",colnames(pred_matrix_scaled),
...
for(i in 3:(2+ncol(pred_matrix_scaled))){
...
initial_set_of_predictors<-colnames(pred_matrix_scaled)
...
while(WAIC_minimized=="No"){
  ...
  for(z in 1:length(pred_comb)){
    ...
    nonstratum_included<-ifelse(!(is.na(new_pred[1])),"Yes","No")
    ...
    # If non-stratum predictors are included, use model with non-stratum predictors.
    if(nonstratum_included=="Yes"){
      ...
      # If non-stratum predictors are not included, use the intercept only model.
    ...
    for(i in 1:nrow(reads)){
      ...
      for(k in 1:ncol(reads)){
        ...
        # If non-stratum predictors are included, add on the effects.
        if(nonstratum_included=="Yes"){
    ...
    # Create a data frame for monitoring the variable selection process.
    ## Create data frame.
    df_new_mod<-as.data.frame(matrix(ncol=2+ncol(pred_matrix_scaled)+5))
    colnames(df_new_mod)<-c("Set","Model",colnames(pred_matrix_scaled),
                    "Num_Rhats_gt_1.05","Num_Eff_SS_ls_100",
                    "Num_params","Run_Time","WAIC")
    ## Add values to the data frame.
    ### Provide the set number.
    ...
    if(nonstratum_included=="Yes"){
      ...
      df_new_mod[,3:(2+ncol(pred_matrix_scaled))]<-0
    }
    ### Provide number of R-hats greater than 1.05.
  ...
  best_fit_model_from_current_set_predictors<-
    best_fit_model_from_current_set[,3:(2+ncol(pred_matrix_scaled))]
...
best_mod_pred<-best_mod[,3:(2+ncol(pred_matrix_scaled))]
...
set_best_mod_pred<-df3[,3:(2+ncol(pred_matrix_scaled))]
